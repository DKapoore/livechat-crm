...
(function () {
  ...
  style.textContent = `
    ...
    #lc-fab {
      ...
      transition: transform .2s ease; overflow: hidden; padding: 0;
    }
    ...
    #lc-fab-icon { display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; }
    #lc-fab-icon.lc-icon-full { width: 100%; height: 100%; }
    #lc-fab-icon.lc-icon-full img { width: 100%; height: 100%; object-fit: cover; display: block; }

    /* ---- Floating button animations ---- */
    @keyframes lc-anim-pulse { 0%, 100% { box-shadow: 0 10px 25px rgba(22,163,74,0.4), 0 0 0 0 var(--lc-brand, #16a34a); } 50% { box-shadow: 0 10px 25px rgba(22,163,74,0.4), 0 0 0 10px rgba(22,163,74,0); } }
    @keyframes lc-anim-bounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
    @keyframes lc-anim-glow { 0%, 100% { box-shadow: 0 10px 25px rgba(22,163,74,0.4); } 50% { box-shadow: 0 10px 35px 8px var(--lc-brand, #16a34a); } }
    @keyframes lc-anim-shake { 0%, 100% { transform: rotate(0deg); } 20% { transform: rotate(-8deg); } 40% { transform: rotate(8deg); } 60% { transform: rotate(-6deg); } 80% { transform: rotate(6deg); } }
    #lc-fab.lc-anim-pulse { animation: lc-anim-pulse 2s ease-in-out infinite; }
    #lc-fab.lc-anim-bounce { animation: lc-anim-bounce 1.4s ease-in-out infinite; }
    #lc-fab.lc-anim-glow { animation: lc-anim-glow 2s ease-in-out infinite; }
    #lc-fab.lc-anim-shake { animation: lc-anim-shake 3.5s ease-in-out infinite; }

    /* ---- Optional label bubble next to the button (positioned via JS, depends on button size) ---- */
    #lc-fab-label {
      position: fixed; z-index: 999998; background: #fff; color: #0f172a; padding: 9px 14px;
      border-radius: 20px; font-size: 13px; font-weight: 600; box-shadow: 0 8px 20px rgba(0,0,0,.15);
      white-space: nowrap; font-family: -apple-system, 'Segoe UI', Roboto, sans-serif; cursor: pointer;
      display: none;
    }
    #lc-fab-label.visible { display: block; }
  ...
  const fabLabelEl = document.getElementById('lc-fab-label');
  ...
  let currentButtonSize = 60;

  // Positions the label bubble beside the fab, on whichever side isn't hugging the screen edge,
  // vertically centered on the button. Recomputed whenever position or size changes.
  function positionLabel() {
    if (!fabLabelEl.classList.contains('visible')) return;
    const gap = 12;
    const cornerOffset = 24;
    const isRight = currentPosition.indexOf('right') !== -1;
    const isTop = currentPosition.indexOf('top') !== -1;
    fabLabelEl.style.top = fabLabelEl.style.bottom = fabLabelEl.style.left = fabLabelEl.style.right = '';
    const verticalOffset = Math.max(cornerOffset + currentButtonSize / 2 - 18, cornerOffset);
    if (isTop) fabLabelEl.style.top = verticalOffset + 'px';
    else fabLabelEl.style.bottom = verticalOffset + 'px';
    if (isRight) fabLabelEl.style.right = (cornerOffset + currentButtonSize + gap) + 'px';
    else fabLabelEl.style.left = (cornerOffset + currentButtonSize + gap) + 'px';
  }
  ...
  // Applies size, animation, outline, label text, and icon display mode from the widget's config.
  function applyButtonStyle(config) {
    const size = config.buttonSize || 60;
    currentButtonSize = size;
    fab.style.width = size + 'px';
    fab.style.height = size + 'px';
    fab.style.fontSize = Math.round(size * 0.43) + 'px';

    ['lc-anim-pulse', 'lc-anim-bounce', 'lc-anim-glow', 'lc-anim-shake'].forEach(c => fab.classList.remove(c));
    if (config.buttonAnimation && config.buttonAnimation !== 'none') {
      fab.classList.add('lc-anim-' + config.buttonAnimation);
    }

    if (config.buttonOutlineColor && config.buttonOutlineWidth > 0) {
      fab.style.border = config.buttonOutlineWidth + 'px solid ' + config.buttonOutlineColor;
    } else {
      fab.style.border = 'none';
    }

    if (config.buttonLabelEnabled && config.buttonLabelText) {
      fabLabelEl.textContent = config.buttonLabelText;
      fabLabelEl.classList.add('visible');
    } else {
      fabLabelEl.classList.remove('visible');
    }
    positionLabel();

    if (config.iconType === 'image' && config.iconDisplayMode === 'full') {
      fabIcon.classList.add('lc-icon-full');
    } else {
      fabIcon.classList.remove('lc-icon-full');
    }
  }
  ...
  fabLabelEl.addEventListener('click', () => (isOpen ? closeChat() : openChat()));
  ...
  loadScript(ORIGIN + '/socket.io/socket.io.js').then(() => {
    ...
    socket.on('widget:ready', (data) => {
      ...
      if (data.company) applyButtonStyle(data.company);
      if (data.company && data.company.iconType === 'image' && data.company.iconValue) {
        ...
        if (data.company.iconDisplayMode === 'full') {
          fabIcon.innerHTML = `<img src="${imgSrc}" alt="" />`; // sizing handled by the .lc-icon-full CSS class
        } else {
