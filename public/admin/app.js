(function () {
  ...
  function connectSocket() {
    ...
    socket.on('queue:new', () => { refreshConvListSilently(); flashNav('inbox'); flashNav('queue'); });
  ...
  async function renderVisitors() {
    ...
    document.getElementById('visitors-table').innerHTML = visitors.length ? `
      <table><thead><tr><th>Name</th><th>Contact</th><th>Page</th><th>First seen</th></tr></thead>
      <tbody>${visitors.map(v => `<tr><td>${escapeHtml(v.name || '—')}</td><td>${escapeHtml(v.contact || '—')}</td><td>${escapeHtml(v.page_url || '—')}</td><td>${timeAgo(v.first_seen)}</td></tr>`).join('')}</tbody></table>
