(function () {
  ...
  function buildWidgetCard(widget, totalWidgetCount) {
    ...
    let selectedIconDisplayMode = widget.icon_display_mode || 'icon';
    let selectedAnimation = widget.button_animation || 'none';
    let selectedLabelEnabled = !!widget.button_label_enabled;
    let selectedLabelText = widget.button_label_text || 'Chat with us!';
    let selectedOutlineColor = widget.button_outline_color || '#ffffff';
    let selectedOutlineWidth = widget.button_outline_width || 0;
    let selectedButtonSize = widget.button_size || 60;
    ...
    wrap.innerHTML = `
      ...
      <div class="two-col" style="grid-template-columns: 1fr 300px;">
        <div>
          ...
          <div class="icon-display-mode-wrap" style="display:none; margin-bottom:20px;">
            <label style="font-size:12px; font-weight:600; color:var(--ink-soft); display:block; margin-bottom:6px;">Image display style</label>
            <div style="display:flex; gap:6px;">
              <button type="button" class="btn icon-display-choice" data-mode="icon" style="flex:1;">🔘 Small icon</button>
              <button type="button" class="btn icon-display-choice" data-mode="full" style="flex:1;">⬤ Full button background</button>
            </div>
          </div>

          <div style="font-size:12px; font-weight:700; color:var(--ink-soft); margin-bottom:8px;">BUTTON ANIMATION</div>
          <div style="display:flex; flex-wrap:wrap; gap:6px; max-width:420px; margin-bottom:20px;">
            ${[['none', '⬜ None'], ['pulse', '💫 Pulse'], ['bounce', '⬆️ Bounce'], ['glow', '✨ Glow'], ['shake', '📳 Shake']].map(([val, label]) => `
              <button type="button" class="btn animation-choice" data-anim="${val}" style="flex:1; min-width:80px;">${label}</button>
            `).join('')}
          </div>

          <label style="display:flex; align-items:center; gap:8px; font-size:12px; font-weight:700; color:var(--ink-soft); margin-bottom:10px; cursor:pointer;">
            <input type="checkbox" class="label-toggle" ${selectedLabelEnabled ? 'checked' : ''} style="width:auto;" />
            TEXT LABEL NEXT TO BUTTON
          </label>
          <div class="label-text-wrap" style="${selectedLabelEnabled ? '' : 'display:none;'} margin-bottom:20px; max-width:380px;">
            <input type="text" class="label-text-input" value="${escapeHtml(selectedLabelText)}" maxlength="40" placeholder="Chat with us!" style="width:100%; padding:8px 10px; border:1px solid var(--border); border-radius:6px; font-size:12.5px; box-sizing:border-box;" />
          </div>

          <div style="font-size:12px; font-weight:700; color:var(--ink-soft); margin-bottom:8px;">OUTLINE</div>
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:20px;">
            <input type="color" class="outline-color-input" value="${selectedOutlineColor}" style="width:50px; height:36px; border:1px solid var(--border); border-radius:6px; cursor:pointer;" />
            <label style="font-size:11.5px; color:var(--ink-soft);">Width</label>
            <input type="range" class="outline-width-input" min="0" max="6" value="${selectedOutlineWidth}" style="width:100px;" />
            <span class="outline-width-value" style="font-size:11.5px; color:var(--ink-soft); width:36px;">${selectedOutlineWidth}px</span>
          </div>

          <div style="font-size:12px; font-weight:700; color:var(--ink-soft); margin-bottom:8px;">BUTTON SIZE</div>
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:20px; max-width:320px;">
            <input type="range" class="size-slider-input" min="44" max="100" value="${selectedButtonSize}" style="flex:1;" />
            <span class="size-slider-value" style="font-size:11.5px; color:var(--ink-soft); width:44px;">${selectedButtonSize}px</span>
          </div>
        ...
        <div>
          ...
          <div class="preview-box" style="position:relative; height:280px; background:#f1f5f9; border:1px solid var(--border); border-radius:12px; overflow:hidden;">
            ...
            <div class="preview-label" style="position:absolute; display:none; background:#fff; padding:6px 10px; border-radius:14px; font-size:11px; font-weight:600; box-shadow:0 4px 10px rgba(0,0,0,.12); white-space:nowrap;"></div>
          </div>
        </div>
      </div>
    `;
    ...
    const previewLabel = wrap.querySelector('.preview-label');
    ...
    const iconDisplayModeWrap = wrap.querySelector('.icon-display-mode-wrap');
    const labelTextWrap = wrap.querySelector('.label-text-wrap');
    const outlineWidthValueEl = wrap.querySelector('.outline-width-value');
    const sizeSliderValueEl = wrap.querySelector('.size-slider-value');
    ...
    function updatePreview() {
      ...
      previewFab.style.width = selectedButtonSize + 'px';
      previewFab.style.height = selectedButtonSize + 'px';
      previewFab.style.fontSize = Math.round(selectedButtonSize * 0.38) + 'px';
      previewFab.style.border = selectedOutlineWidth > 0 ? `${selectedOutlineWidth}px solid ${selectedOutlineColor}` : 'none';
      previewFab.style.overflow = 'hidden';

      if (selectedIconType === 'image' && selectedIconValue) {
        const imgStyle = selectedIconDisplayMode === 'full' ? 'width:100%;height:100%;object-fit:cover;' : 'width:60%;height:60%;object-fit:cover;border-radius:50%;';
        previewFab.innerHTML = `<img src="${selectedIconValue}" style="${imgStyle}" />`;
      } else {
        previewFab.textContent = selectedIconValue;
      }

      const animMap = { pulse: 'anim-pulse', bounce: 'anim-bounce', glow: 'anim-glow', shake: 'anim-shake' };
      ['anim-pulse', 'anim-bounce', 'anim-glow', 'anim-shake'].forEach(c => previewFab.classList.remove(c));
      if (animMap[selectedAnimation]) previewFab.classList.add(animMap[selectedAnimation]);
      ...
      wrap.querySelectorAll('.icon-display-choice').forEach(btn => btn.classList.toggle('primary', btn.dataset.mode === selectedIconDisplayMode));
      wrap.querySelectorAll('.animation-choice').forEach(btn => btn.classList.toggle('primary', btn.dataset.anim === selectedAnimation));
      ...
      iconDisplayModeWrap.style.display = selectedIconType === 'image' ? 'block' : 'none';
      labelTextWrap.style.display = selectedLabelEnabled ? 'block' : 'none';
      outlineWidthValueEl.textContent = selectedOutlineWidth + 'px';
      sizeSliderValueEl.textContent = selectedButtonSize + 'px';

      if (selectedLabelEnabled && selectedLabelText) {
        previewLabel.textContent = selectedLabelText;
        previewLabel.style.display = 'block';
        const isRight = selectedPosition.indexOf('right') !== -1;
        const isTop = selectedPosition.indexOf('top') !== -1;
        previewLabel.style.top = previewLabel.style.bottom = previewLabel.style.left = previewLabel.style.right = '';
        if (isTop) previewLabel.style.top = '10px'; else previewLabel.style.bottom = '10px';
        if (isRight) previewLabel.style.right = (10 + selectedButtonSize + 10) + 'px'; else previewLabel.style.left = (10 + selectedButtonSize + 10) + 'px';
      } else {
    ...
    wrap.querySelectorAll('.icon-display-choice').forEach(btn => {
      btn.addEventListener('click', () => { selectedIconDisplayMode = btn.dataset.mode; updatePreview(); });
    });
    wrap.querySelectorAll('.animation-choice').forEach(btn => {
      btn.addEventListener('click', () => { selectedAnimation = btn.dataset.anim; updatePreview(); });
    });
    ...
    wrap.querySelector('.label-toggle').addEventListener('change', (e) => { selectedLabelEnabled = e.target.checked; updatePreview(); });
    wrap.querySelector('.label-text-input').addEventListener('input', (e) => { selectedLabelText = e.target.value; updatePreview(); });
    wrap.querySelector('.outline-color-input').addEventListener('input', (e) => { selectedOutlineColor = e.target.value; updatePreview(); });
    wrap.querySelector('.outline-width-input').addEventListener('input', (e) => { selectedOutlineWidth = parseInt(e.target.value, 10) || 0; updatePreview(); });
    wrap.querySelector('.size-slider-input').addEventListener('input', (e) => { selectedButtonSize = parseInt(e.target.value, 10) || 60; updatePreview(); });
    ...
    wrap.querySelector('.save-widget-btn').addEventListener('click', async () => {
      ...
      await api(`/widgets/${widget.id}`, {
        ...
        body: JSON.stringify({
          ...
          icon_display_mode: selectedIconDisplayMode,
          button_animation: selectedAnimation,
          button_label_enabled: selectedLabelEnabled,
          button_label_text: selectedLabelText || 'Chat with us!',
          button_outline_color: selectedOutlineColor,
          button_outline_width: selectedOutlineWidth,
  ...
  async function renderAccount() {
    ...
    if (tg.configured && !tg.myTelegramLinked) {
      document.getElementById('telegram-get-code-btn').addEventListener('click', async () => {
        ...
        document.getElementById('telegram-link-instructions').innerHTML = `
          <div class="card">
            ...
            <div style="display:flex; align-items:center; gap:8px;">
              <div class="embed-box" style="font-size:14px; flex:1;">${escapeHtml(fullCommand)}</div>
              <button class="btn copy-link-code-btn" style="flex-shrink:0;">📋 Copy</button>
            </div>
        ...
        `;
        document.querySelector('.copy-link-code-btn').addEventListener('click', () => {
          navigator.clipboard.writeText(fullCommand);
          const btn = document.querySelector('.copy-link-code-btn');
          const original = btn.textContent;
          btn.textContent = '✓ Copied';
          setTimeout(() => (btn.textContent = original), 1500);
        });
