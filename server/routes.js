...
router.patch('/widgets/:id', (req, res) => {
  ...
  if (!widget) return res.status(404).json({ error: 'Not found' });
  const {
    name, brand_color, welcome_message, widget_position, icon_type, icon_value,
    lead_form_enabled, lead_form_services, lead_form_consent_text,
    icon_display_mode, button_animation, button_label_enabled, button_label_text,
    button_outline_color, button_outline_width, button_size,
  } = req.body || {};
  ...
  const VALID_ANIMATIONS = ['none', 'pulse', 'bounce', 'glow', 'shake'];
  const animation = VALID_ANIMATIONS.includes(button_animation) ? button_animation : widget.button_animation;
  const displayMode = icon_display_mode === 'full' ? 'full' : (icon_display_mode === 'icon' ? 'icon' : widget.icon_display_mode);
  const size = button_size !== undefined ? Math.max(44, Math.min(100, parseInt(button_size, 10) || widget.button_size)) : widget.button_size;
  const outlineWidth = button_outline_width !== undefined ? Math.max(0, Math.min(6, parseInt(button_outline_width, 10) || 0)) : widget.button_outline_width;
  ...
  `).run(
      ...
      displayMode,
      animation,
      button_label_enabled !== undefined ? (button_label_enabled ? 1 : 0) : widget.button_label_enabled,
      button_label_text ?? widget.button_label_text,
      button_outline_color !== undefined ? button_outline_color : widget.button_outline_color,
      outlineWidth,
      size,
      widget.id
    );
