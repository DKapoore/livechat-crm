...
router.get('/visitors/export.csv', (req, res) => {
  ...
  const header = ['Name', 'Contact', 'Page URL', 'First Seen', 'Last Seen'];
  const rows = visitors.map(v => [v.name, v.contact, v.page_url, v.first_seen, v.last_seen].map(escape).join(','));
...
router.patch('/widgets/:id', (req, res) => {
  ...
  const widget = db.prepare(`SELECT * FROM widgets WHERE id=? AND company_id=?`).get(req.params.id, req.auth.companyId);
  if (!widget) return res.status(404).json({ error: 'Not found' });
  const { name, brand_color, welcome_message, widget_position, icon_type, icon_value } = req.body || {};
  ...
  db.prepare(`UPDATE widgets SET name=?, brand_color=?, welcome_message=?, widget_position=?, icon_type=?, icon_value=? WHERE id=?`)
    .run(
