...
function getWidgetName(conversation) {
  if (!conversation.widget_id) return null;
  try {
    const widget = db.prepare(`SELECT name FROM widgets WHERE id = ?`).get(conversation.widget_id);
...
async function notifyAgentAssigned(company, agent, conversation, visitor) {
  ...
  const widgetName = getWidgetName(conversation);
  const visitorLabel = widgetName
    ? `${widgetName} visitor (${visitor.name || 'Anonymous'})`
    : (visitor.name || 'Anonymous visitor');
  const text =
    `🟢 <b>New chat assigned to you</b>\n` +
    `${escapeHtml(visitorLabel)}\n` +
...
async function forwardVisitorMessage(company, agent, conversation, visitorMsg, origin) {
  if (!company.telegram_bot_token || !agent.telegram_chat_id) return;
  const widgetName = getWidgetName(conversation);
  const visitorName = visitorMsg.sender_name || 'Anonymous';
  const visitorLabel = widgetName ? `${widgetName} visitor (${visitorName})` : visitorName;
  const replyHint = `<i>Reply with #${conversation.short_code} your message</i>`;
  ...
  if (visitorMsg.attachment_url) {
    ...
    const caption = `<b>${escapeHtml(visitorLabel)}</b> sent an attachment${visitorMsg.text ? ':\n' + escapeHtml(visitorMsg.text) : ''}\n\n${replyHint}`;
    ...
    await sendTelegramMessage(company.telegram_bot_token, agent.telegram_chat_id,
      `💬 <b>${escapeHtml(visitorLabel)}</b> sent an attachment (couldn't preview it here): ${absoluteUrl}\n\n${replyHint}`);
    return;
  }

  const text = `💬 <b>${escapeHtml(visitorLabel)}</b> says:\n${escapeHtml(visitorMsg.text || '')}\n\n${replyHint}`;
  await sendTelegramMessage(company.telegram_bot_token, agent.telegram_chat_id, text);
}
