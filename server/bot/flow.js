...
// Used when a visitor submits the mandatory Lead Capture Form (name/mobile/email/interest/consent)
// instead of going through the chat-style name/contact questions. Any bot_questions with
// field_key 'name' or 'contact' are skipped since the form already collected that.
function startConversationFromLead(companyId, pageUrl, widgetId, leadData) {
  const visitorId = randomUUID();
  db.prepare(`
    INSERT INTO visitors (id, company_id, page_url, name, contact, mobile, email, interested_services, consent_given, lead_captured_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
  `).run(
    visitorId, companyId, pageUrl || null,
    (leadData.name || '').slice(0, 80),
    leadData.email || leadData.mobile || null,
    leadData.mobile || null,
    leadData.email || null,
    JSON.stringify(leadData.interestedServices || []),
  ...
  const shortCode = generateShortCode(companyId);
  const questions = getQuestions(companyId);
  let startStep = questions.findIndex(q => q.field_key !== 'name' && q.field_key !== 'contact');
  if (startStep === -1) startStep = questions.length;

  db.prepare(`INSERT INTO conversations (id, company_id, widget_id, short_code, visitor_id, status, bot_step)
              VALUES (?, ?, ?, ?, ?, 'bot', ?)`)
    .run(conversationId, companyId, widgetId, shortCode, visitorId, startStep);

  const widget = widgetId ? db.prepare(`SELECT * FROM widgets WHERE id = ?`).get(widgetId) : null;
  const welcomeText = widget ? widget.welcome_message : 'Welcome! 👋';
  const messages = [saveMessage(conversationId, 'bot', `${welcomeText} Thanks, ${leadData.name}!`)];

  const nextQuestion = questions[startStep];
  if (nextQuestion) {
    messages.push(askQuestion(conversationId, nextQuestion));
  } else {
    db.prepare(`UPDATE conversations SET status = 'waiting' WHERE id = ?`).run(conversationId);
    messages.push(saveMessage(conversationId, 'bot', "I'm connecting you with the best available agent. Please hold on. 🙏"));
  }
...
module.exports = { handleVisitorMessage, startConversation, startConversationFromLead, autoAssign, saveMessage, getQuestions };
