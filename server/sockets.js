    ...
    socket.on('widget:init', ({ widgetKey, pageUrl, conversationId }) => {
      ...
      if (conversation) {
        ...
        socket.emit('widget:ready', {
          company: { name: company.name, brandColor: widget.brand_color, position: widget.widget_position || 'bottom-right', iconType: widget.icon_type || 'emoji', iconValue: widget.icon_value || '💬' },
