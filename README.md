...
## New in this update (v5)

- **Advanced button styling** (Admin → Widget Customizer → any widget):
  - **Animation**: None, Pulse, Bounce, Glow, or Shake — a subtle looping effect to draw attention to the button.
  - **Text label**: an optional speech-bubble-style label next to the button (e.g. "Chat with us!"), positioned automatically on whichever side doesn't hug the screen edge.
  - **Full-image background mode**: when using an uploaded image icon, choose between a small centered icon or having the image fill the entire button as a circular background/logo.
  - **Outline**: a configurable border color and width (0–6px) around the button.
  - **Size slider**: button diameter from 44px to 100px.
  All of these are live in the Widget Customizer's preview panel and take effect on your site immediately after saving — no code changes.

## New in this update (v4)

- **Lead Capture Form**: per-widget optional toggle (Admin → Widget Customizer → any widget →
  "Lead Capture Form") that shows a proper form — Name, Mobile, Email (all mandatory), a
  multi-select "Interested In" dropdown (options you configure), and a required consent
  checkbox — before the chat opens. Once submitted, the bot flow skips asking name/contact
  again and picks up at the next configured question (e.g. category selection), if any.
- **Email notifications (SMTP, optional)**: Admin → Settings → Email Notifications. Configure
  any SMTP provider (Gmail, SendGrid, etc.) and get emailed the full lead details the moment
  a visitor submits the form, plus a "Send test email" button to verify it works. Agents can
  also get emailed automatically when a chat is assigned to them.
- **Google Sheets sync (optional)**: Admin → Settings → Google Sheets Sync. Every new lead is
  pushed live into a "Leads" tab in a Google Sheet you control, via a small Google Apps Script
  you deploy yourself (code + Hinglish setup guide included in `google-apps-script/`). A
  "Sync Now" button also mirrors agents (name/email/role/status — never passwords),
  departments, widgets, and dashboard stats into their own tabs.
  **Security note**: passwords, SMTP credentials, and Telegram bot tokens are never sent to
  the Sheet — this was verified with an automated test. The Sheet is a one-way export/backup;
  the admin panel's database stays the source of truth and does not read from the Sheet.
- Visitors page and CSV export now include mobile, email, interested services, and consent.
